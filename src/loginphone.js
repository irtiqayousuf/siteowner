...
const useStyles = makeStyles((theme) => ({
  ...
  log: {
    backgroundImage:
      'url(https://images.pexels.com/photos/12040331/pexels-photo-12040331.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1)',
    height: '650px',
...
const LoginPhone = () => {
  ...
  const handleClick = (id) => {
    ...
    if (id === '1') {
      axios
        .post('http://localhost:8080/api/verifyphone', { phone }, config)
        .then((resp) => {
      ...
      axios
        .post('http://localhost:8080/api/verifyapp', { phone }, config)
        .then((resp) => {
          console.log('res', resp.data);
          const res = resp.data;
          // if (!res.error && res.status === 200) {
        ...
        <Box direction="row" sx={{ paddingBlock: '8px' }}>
          <Toolbar>
            ...
            <Stack spacing={2} direction="row">
              <Button type="submit" color="primary" variant="contained" onClick={() => history.push('/')}>
                Back
              </Button>
            </Stack>
          </Toolbar>
